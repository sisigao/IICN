    ...
    def _page_layer(self, embedding_layer):
        ...
        if self.page_layer == 'dynamic_page':
            ...
            for ii in range(page_size):
                ...
                # 创建修改后的页面广告表示
                modified_page = current_page.clone()
                ...
                for b in range(batch_size):
                    ...
                    for click_idx in click_indices:
                        ...
                        # 1. 计算未点击物品的归一化权重 (仅对高相似度未点击物品)
                        non_click_weights = F.softmax(non_click_sims[high_sim_mask], dim=0)

                        # 2. 计算未点击物品的加权和
                        non_click_vectors = current_page[b, high_sim_indices]  # [K, D]
                        weighted_non_click = torch.sum(non_click_weights.view(-1, 1) * non_click_vectors, dim=0)  # [D]

                        # 3. 获取原始点击物品向量
                        click_vector = current_page[b, click_idx]  # [D]

                        # 4. 计算四个特征向量
                        # a. 原始点击物品向量
                        vec1 = click_vector
                        # b. 未点击物品的加权和向量
                        vec2 = weighted_non_click
                        # c. 点击-未点击的差向量
                        vec3 = click_vector - weighted_non_click
                        # d. 点击×未点击的元素积向量
                        vec4 = click_vector * weighted_non_click

                        # 5. 拼接四个向量形成新的点击物品表示
                        new_click_vector = torch.cat([vec1, vec2, vec3, vec4], dim=0)  # [4*D]

                        # 6. 更新页面中的点击物品表示
                        # 注意：我们保留原始维度，通过线性变换将4*D映射回D
                        modified_page[b, click_idx] = self.click_vector_transform(new_click_vector)

                # 使用修改后的页面表示继续后续计算
                current_page = modified_page
                # ================ 新增功能结束 ================

                # 使用原始注意力权重（注意：广告向量已修改）
