...
def Run(DataSettings, ModelSettings, TrainSettings, ResultSettings,
    ...
    if mode == "train":  # train mode
        ...
        for epoch_i in range(start_epoch, total_epoch):
            ...
            ### test
            result, tmp_auc, tmp_f1, tmp_recall, tmp_mse = Engine.evaluate(Sampler.Val_data, epoch_i)
            ### early stop
            if tmp_auc > val_auc:
                val_auc = tmp_auc
                endure_count = 0
                ...
                log.info(f"AUC: {tmp_auc}, F1: {tmp_f1}, Recall: {tmp_recall}, MSE: {tmp_mse}")
                # save model file
        ...
        Engine.model = torch.load(f'{model_save_dir}{model_name}.pt').to(TrainSettings['device'])
        result, _, _, _, _ = Engine.evaluate(Sampler.Test_data, epoch_id=0)
        test_result = result
        log.info('test results( timestamp: ' + str(int(timestamp)) + '):\n' + test_result + '\n')
